...
import Ayurnavbar from "./Ayurnavbar"; // 👈 Global Navbar integrated
import "../style.css"; 
...
export default function MyConsultations() {
  ...
  const [user, setUser] = useState(null); // 👈 Navbar avatar initials ke liye
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      ...
      try {
        const res = await axios.get("http://localhost:5000/api/consultation/my-bookings", {
  ...
  return (
    <div className="home-page-wrapper" style={{ background: '#022417', minHeight: '100vh', width: '100vw', overflowX: 'hidden' }}>
      ...
      {/* ── UPDATED: New Modern Ayurnavbar ── */}
      <Ayurnavbar user={user} onLogout={handleLogout} />

      {/* --- CONTENT AREA --- */}
      {/* 120px padding-top ensures content starts after the fixed navbar */}
