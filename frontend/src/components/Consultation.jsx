...
export default function Consultation() {
  ...
  useEffect(() => {
    if (selectedDate && selectedExpert) {
      setBusySlots([]); // Clear old slots while loading new ones
      axios.get(`http://localhost:5000/api/consultation/busy-slots?practitioner=${encodeURIComponent(selectedExpert.name)}&date=${selectedDate}`)
    ...
    <div className="home-page-wrapper">
              ...
              <div key={expert.id} className="dashboard-row-item">
                <div className="dash-meta-grid">
                  ...
                  <div className="dash-item-btn">
                    <button className="parrot-outline-btn" onClick={() => { setSelectedExpert(expert); setShowModal(true); }}>Book Slot</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      {showModal && (
        <div className="modal-overlay" style={{ zIndex: 2000 }}>
          <div className="modal-content" style={{ maxWidth: '550px', padding: '35px', background: '#0a2113' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              ...
              <div className="input-field-wrapper">
                <label>Date</label>
                <input type="date" className="custom-input" min={new Date().toISOString().split("T")[0]} value={selectedDate} onChange={(e) => { setSelectedDate(e.target.value); setSelectedSlot(""); }} />
              </div>
              <div className="input-field-wrapper">
                <label>Time Slot</label>
                <select className="custom-select" value={selectedSlot} onChange={(e) => setSelectedSlot(e.target.value)} disabled={!selectedDate} style={{ background: '#05120a', color: 'white' }}>
                  <option value="">{selectedDate ? "Choose" : "Pick Date First"}</option>
                  {timeSlots.map(slot => {
                    // Optimized string comparison
                    const isBusy = busySlots.some(b => b.trim().toLowerCase() === slot.trim().toLowerCase());
                    return (
                      <option key={slot} value={slot} disabled={isBusy} style={{ color: isBusy ? '#ff4d4d' : 'white', backgroundColor: isBusy ? '#1a1a1a' : '#0a2113' }}>
