...
export default function Consultation() {
  ...
  const [selectedRefIndex, setSelectedRefIndex] = useState(""); // Stores the index from history array

  const isActive = (path) => location.pathname === path;

  // Available slots from 10 AM to 5 PM
  const timeSlots = [
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        // Reverse so Ref #3 is at index 0, but logic handles actual ID mapping
  ...
  const confirmBooking = async () => {
    ...
    const actualRefId = history.length - selectedRefIndex; // Mapping back to visual Ref ID
    ...
    <div className="home-page-wrapper">
      <nav className="home-nav-dark">
        ...
        <div className="nav-right">
          <button onClick={() => { localStorage.removeItem("token"); navigate("/login"); }} className="logout-btn-light">Logout</button>
        </div>
      </nav>
              ...
              <div key={expert.id} className="dashboard-row-item">
                <div className="dash-meta-grid">
                  ...
                  <div className="dash-item">
                    ...
                    <p className="text-white" style={{ fontWeight: '600' }}>{expert.name}</p>
                  </div>
                  <div className="dash-item">
                  ...
                  <div className="dash-item-btn">
                    <button className="parrot-outline-btn" style={{ borderRadius: '50px', padding: '10px 30px' }} onClick={() => handleBookClick(expert)}>
                      Book Slot
                    </button>
                  </div>
                </div>
      ...
      {/* Restricted Selection Modal */}
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '500px', padding: '35px' }}>
            <h2 className="title-white" style={{ marginBottom: '20px' }}>Schedule Slot</h2>
